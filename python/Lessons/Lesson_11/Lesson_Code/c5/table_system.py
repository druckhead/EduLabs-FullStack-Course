...
class Table:
    ...
    def reserve(self, guests_num) -> None:

        # we allow to reserve a table only if it's available
        # and amount fo guests fits the amount of seats
        if not (self.occupied_seats == 0):
            raise TableIsAlreadyReservedError()
        if self.seats < guests_num:
            raise TableOverbookedError()
    ...
    def release(self) -> None:
        ...
        if self.occupied_seats == 0:
            raise TableIsAlreadyAvailableError
...
class TableReservationSystem:
    ...
    def reserve(self, guests_num, table_id) -> None:
        for table in self.tables:
            if table.table_id == table_id:
                table.reserve(guests_num)
